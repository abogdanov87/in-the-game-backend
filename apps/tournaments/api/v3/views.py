...
class MatchesAPIView(generics.RetrieveAPIView):
    queryset = Tournament.objects.all()
    serializer_class = TournamentMatchesSerializer
    # permission_classes = [permissions.IsAuthenticated]
    permission_classes = [permissions.AllowAny]
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        qs = Forecast.objects.filter(
            ...
            tournament=kwargs['pk'],
            user=6,
        )
        matches = []
        for match in response.data['matches']:
            match['forecast'] = ForecastShortSerializer(qs.filter(match=match['id']).first()).data
            matches += [match]
